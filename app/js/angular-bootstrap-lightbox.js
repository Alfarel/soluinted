...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.prevImage() ng-swipe-right=Lightbox.nextImage()><div class=lightbox-nav><div class=btn-group><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage()>‹ Anterior</a><a ng-href={{Lightbox.image.url}} target=_blank class=\"btn btn-xs btn-default\" title=\"Open in new tab\">Abrir en nueva pestaña</a><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.nextImage()>Siguiente ›</a></div></div><div class=lightbox-image-container><img lightbox-src={{Lightbox.image.url}} alt=\"\"><div class=modal-footer><button class=\"btn  btn-primary\" ng-click=$dismiss()>Cerrar</button></div></div></div>");
